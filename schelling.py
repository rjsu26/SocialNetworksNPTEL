...
def display_graph(G, pos, labels, type1_node_list, type2_node_list, empty_cells, figure_number):
...
def  make_a_node_satisfied(G, labels, unsatisfied_nodes_list, empty_cells):
    if len(unsatisfied_nodes_list ) !=0:
        ...
        G.node[new_position]['type'] , G.node[node_to_shift]['type'] = G.node[node_to_shift]['type'] , 0
...
def iterate(N,cycles,t):
    ...
    display_graph(G, pos, labels, type1_node_list, type2_node_list, empty_cells, "Before Iteration")
    ...
    for i in range(cycles):
        ...
        satisfied_nodes_list = list(set(G.nodes())-set(unsatisfied_nodes_list) - set(empty_cells))
        #Generating 100 data points for satisfaction% corresponding to time
        intervals = int(cycles/100)
        if i%intervals==0:
            satisfaction_percent = 100 * len(satisfied_nodes_list) / (N*N - len(empty_cells))
            x_axis.append(i)
        ...
        generate_all_3_node_list(G,type1_node_list,type2_node_list,empty_cells)
        if i==1:
            print("Starting operation of ",N,' x ',N,' nodes with ', cycles,' cycles..')
            a = time.time()
        if i == 100:
            b=time.time()
            time_taken = float("{0:.2f}".format(b-a))
            print("estimated time lapsed: ", time_taken, " seconds...")
            print('estimated time remaining: ', float("{0:.2f}".format(time_taken*cycles_fraction_left*cycles/100)) , " seconds..")
            time.sleep(3)

    display_graph(G, pos, labels, type1_node_list, type2_node_list, empty_cells,"After Iteration")    
    print("Task complete")
